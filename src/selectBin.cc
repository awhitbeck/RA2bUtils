...
public : 
  ...
  double minMHT,maxMHT,minHT,maxHT,minPhotons,maxPhotons;
  ...
  selectBin( TreeType *ntuple_ ,
	     ...
	     int minPhotons_ = 0 , int maxPhotons_ = 99 )
    : processor<TreeType>("selectBin_"+binLabel_)
  {
    ...
    minVTags = minVTags_;
    maxVTags = maxVTags_;
    minPhotons = minPhotons_; 
    maxPhotons = maxPhotons_;
  ...
  bool process( ) override {
    ...
    */
    ...
    if( ntuple->NumPhotons>=minPhotons && ntuple->NumPhotons<=maxPhotons ) histo->Fill(6);
    else return false;
    ...
    for( unsigned int j = 0 ; j < ak8Jets.size() ; ++j ){
      ...
      */
      ...
      if( ntuple->JetsAK8_bDiscriminatorSubjet1CSV->at(j) > .605 &&
	  ntuple->JetsAK8_bDiscriminatorSubjet2CSV->at(j) > .605 &&
    ...
    if( numHiggsTags>=minHiggsTags && numHiggsTags<=maxHiggsTags ) histo->Fill(7);
    else return false;
    if( numVTags>=minVTags && numVTags<=maxVTags ) histo->Fill(8);
    else return false;
